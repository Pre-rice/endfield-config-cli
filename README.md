...
3. 建议先**退出游戏**再运行（终末地只在**启动时读取**注册表，游戏运行中修改需重启才生效；游戏退出时**不会**覆盖注册表里的修改，脚本检测到游戏运行会给出提示）。
4. 运行：
...
**画面质量与单项设置**：设置**受画面质量预设控制的 9 项**（全局特效质量、阴影质量、体积雾、体积云、环境光遮蔽、场景细节、环境细节、植被密度、屏幕空间反射）时，脚本会自动把画面质量切到**自定义档**（写 `video_custom_quality=1`），这些单项才生效；未设置的单项沿用游戏内当前值。**只设置「画面质量」预设档**时脚本切回预设档（写 `custom=0`），一次性应用整套预设。其余画质键（帧率、纹理质量、各向异性采样、垂直同步、色差、队友技能特效质量、画质提升、DLSS超分辨模式、锐化程度、帧生成、DLSS帧生成模式、NVIDIA Reflex、接触阴影）及声音、语言设置均为**独立设置**，不受画面质量档位影响，随时可改。
...
- **单项画质键在「自定义」档下才生成**：受画面质量预设控制的 9 项（全局特效质量、阴影质量、体积雾、体积云、环境光遮蔽、场景细节、环境细节、植被密度、屏幕空间反射）只在「画面质量 = 自定义」并在游戏中调整过对应项后才会出现在注册表中；不调整则不生成。各键档位映射见上文表格。
- **画面质量档位与自定义**：游戏内切「自定义」时 `video_quality_main` 保留原预设档位的值，自定义状态由单独的状态键 `video_custom_quality` 表达（0=预设档 / 1=自定义档）。脚本设置**单项画质**时自动置自定义档，**只设画面质量**时切回预设档（见上文「画面质量与单项设置」）。
- **游戏运行时行为**：终末地在运行中把游戏内设置的改动**实时写回注册表**，但它只在**启动时读取**注册表。因此在游戏运行中用脚本修改设置是**安全**的（不会被游戏退出覆盖），只是需要**重启游戏**才会生效；若之后在游戏内调整设置，会实时写回覆盖脚本刚写入的对应键。
- **未知设置名**：配置文件里写了表格之外的键名会报错。
- 脚本不会删除注册表中的未知 / 未来字段，只会按需追加或修改，保证兼容性。

### 无法外部修改：音量与队友技能特效不透明度

**音量（总音量 / 语音音量 / 音乐音量 / 音效音量）**不存储在注册表中，本脚本无法修改。经多轮实验确认：
...
**队友技能特效不透明度**（`teammate_skill_effect_opacity`）同样无法外部修改：该键在游戏运行时读到的值随引擎状态变化、不反映滑块数值（调 0.8 时读到 `0xA0000000`、调 1.0 时读到 `0`），判定与音量键同类。脚本不包含该项，只能在**游戏内**调整；上文表格保留该行仅为对照游戏内设置顺序。
...
| `--wait-game` | 若终末地正在运行，等待其退出后再应用设置（避免与游戏内设置的实时写回冲突） |
