...
| 内部键名 | 官方中文名 | 合法取值 |
|---|---|---|
| `video_quality_main` | 画面质量 | `极低` / `低` / `中` / `高` / `极高` / `自定义`（自定义档只切状态键、不改主键） |
...
**画面质量与单项设置**：设置**受画面质量预设控制的 9 项**（全局特效质量、阴影质量、体积雾、体积云、环境光遮蔽、场景细节、环境细节、植被密度、屏幕空间反射）时，脚本会自动把画面质量切到**自定义档**（写 `video_custom_quality=1`），这些单项才生效；未设置的单项沿用游戏内当前值。**只设置「画面质量」预设档**（`极低`~`极高`）时脚本切回预设档（写 `custom=0`），一次性应用整套预设。**设置「画面质量 = `自定义`」**时脚本同样切到自定义档（写 `custom=1`），但**不修改** `video_quality_main` 主键（保留当前档位值）——效果与直接设置单项一致，适合只单独微调某项的写法。其余画质键（帧率、纹理质量、各向异性采样、垂直同步、色差、队友技能特效质量、画质提升、DLSS超分辨模式、锐化程度、帧生成、DLSS帧生成模式、NVIDIA Reflex、接触阴影）及声音、语言设置均为**独立设置**，不受画面质量档位影响，随时可改。
...
- **画面质量档位与自定义**：游戏内切「自定义」时 `video_quality_main` 保留原预设档位的值，自定义状态由单独的状态键 `video_custom_quality` 表达（0=预设档 / 1=自定义档）。脚本设置**单项画质**或显式设**「画面质量=自定义」**时自动置自定义档（后者不写主键），**只设画面质量预设档**时切回预设档（见上文「画面质量与单项设置」）。
