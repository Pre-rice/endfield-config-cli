...
在**游戏外**直接修改《明日方舟：终末地》的画面、分辨率、音量、语言等设置。编辑配置文件 → 运行脚本 → 自动写入，无需进游戏手动调。不含解帧功能。
...
| `audio_global` | 总音量 | 0 ~ 10（反向存储） |
| `audio_voice` | 语音音量 | 0 ~ 10（反向存储） |
| `audio_music` | 音乐音量 | 0 ~ 10（反向存储） |
| `audio_sfx` | 音效音量 | 0 ~ 10（反向存储） |
...
- **音量（总/语音/音乐/音效）反向存储**：注册表存的值 = 10 − 游戏内滑块值。游戏内填 8 → 注册表存 2；游戏内显示 10 时注册表存 0。
...
- **部分单项画质注册表无键**：阴影质量、体积雾、体积云、环境光遮蔽、场景细节、环境细节、植被密度、屏幕空间反射、全局特效质量、队友技能特效质量在注册表中没有对应键（推测仅在「画面质量 = 自定义」时由游戏生成），脚本暂不在配置中暴露。若切换到「自定义」后游戏生成了这些键，可告知脚本维护者补全。
